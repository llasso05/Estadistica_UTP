...
boxplot(real_estate$price, main = "Diagrama de caja de los precios de venta", ylab = "Precio de venta")
primer_cuartil <- quantile(real_estate$price, 0.25)
tercer_cuartil <- quantile(real_estate$price, 0.75)
...
cor_size_price <- cor(real_estate$size, real_estate$price)
...
plot(real_estate$distance_to_city, real_estate$price, main = "Dispersión del precio vs. distancia al centro de la ciudad",
     xlab = "Distancia al centro de la ciudad (millas)", ylab = "Precio de venta")
cor_distance_price <- cor(real_estate$distance_to_city, real_estate$price)
...
current_year <- 2024
baseball_2009$stadium_age <- current_year - baseball_2009$year_built
...
boxplot(baseball_2009$salary, main = "Diagrama de caja de los salarios de los equipos", ylab = "Salario del equipo")
primer_cuartil_salary <- quantile(baseball_2009$salary, 0.25)
tercer_cuartil_salary <- quantile(baseball_2009$salary, 0.75)
datos_atipicos_salary <- boxplot.stats(baseball_2009$salary)$out
...
salario_yankees <- baseball_2009$salary[baseball_2009$team == "New York Yankees"]
...
plot(baseball_2009$salary, baseball_2009$games_won, main = "Dispersión del salario del equipo vs. juegos ganados",
     xlab = "Salario del equipo", ylab = "Número de juegos ganados")
cor_salary_wins <- cor(baseball_2009$salary, baseball_2009$games_won)
...
plot(baseball_2009$games_won, main = "Diagrama de puntos del número de juegos ganados",
     xlab = "Índice del equipo", ylab = "Número de juegos ganados", pch = 19)
summary(baseball_2009$games_won)
...
boxplot(school_buses$maintenance_cost, main = "Diagrama de caja del costo de mantenimiento", ylab = "Costo de mantenimiento")
primer_cuartil_mantenimiento <- quantile(school_buses$maintenance_cost, 0.25)
tercer_cuartil_mantenimiento <- quantile(school_buses$maintenance_cost, 0.75)
datos_atipicos_mantenimiento <- boxplot.stats(school_buses$maintenance_cost)$out
...
costo_mediano <- median(school_buses$maintenance_cost)
school_buses$high_cost <- school_buses$maintenance_cost > costo_mediano
tabla_contingencias <- table(school_buses$manufacturer, school_buses$high_cost)
